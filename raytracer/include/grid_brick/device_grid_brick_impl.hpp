...
namespace rt {
  ...
  inline float CDeviceBrickGrid::lookupMajorant(const glm::vec3& ipos, int mip) const {
    ...
    return m_volDensityScale * tex3DLod<float2>(m_rangeObj, (brick.x + 0.5f) / m_rangeSize.x, (float)brick.y / m_rangeSize.y, (float)brick.z / m_rangeSize.z, mip).y; // Seems like CUDA doesn't support half2 lookups
  }
