...
namespace rt {
  ...
  std::tuple<std::vector<SAABB>, std::vector<SAABB>, glm::mat4x3, std::string, glm::vec3> CHostScene::getObjectBBs(ESceneobjectMask mask) const {
    ...
    return { modelBBs, worldBBs, worldToModel, filename, orientation};
  }
}